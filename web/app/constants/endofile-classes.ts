 ...
 * in the deployed graph's output tensor (`web/public/model_proto`, currently `[-1, 29]`), so
 * this list must never be sorted, filtered or reordered by hand — only replaced wholesale,
 * copied straight from the retrained model's own `class_names` (this is the v2 model,
 * trained by `model/endox_ia_reduced_v2.ipynb`, cell 8). `class_names` comes from
 * `tf.keras.utils.image_dataset_from_directory`, which sorts subdirectory names
 * lexicographically — that is why `micromega-remover` sits between
 * `micromega-one-curve-mini` and `rising` below instead of at the end.
 */
